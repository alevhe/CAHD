...
if __name__ == "__main__":
    dim_finale = 1000
    num_sensibile_list = [25, 50, 75, 100]
        ...
        for att in range(max_attempts):
            for index_grado_privacy in range(len(grado_privacy_list)):
                ...
                for index_num_sensibile in range(len(num_sensibile_list)):
                    print("%.2f |File : %s | Index privacy : %s | Attempt : %s | Index sensitive: %s" % (
                        time.time() - big_ben, index_nameFile, index_grado_privacy, att, index_num_sensibile))
                    ...
                    if cahd.CAHD_algorithm(analysis=True,
                        ...
                        if cahd.lista_gruppi is not None:
                            ...
                            file_1 = open("MainPlotData/1/" + name_file + ".txt", "a")
                            file_1.write(str(grado_privacy) + "," + str(num_sensibile) + "," + str(KL_Divergence) + ";")
                            file_1.close()
